...
| `articles/*.json` | 手動深掘り記事のソース（統一JSONスキーマ） |
| `articles/*.html` | 自動生成される詳細ページ |
| `update_news.py` | **パイプライン段階1**: 外部RSS取得 + `source_type: "rss"` |
| `build_articles.py` | **パイプライン段階2**: ローカルJSON記事ビルド + `source_type: "local"` のマージ |
...
`articles/` ディレクトリに新しい `.json` ファイルを作成します。
...
```json
{
  ...
  "category": "AI",
  "score": 5,
  "description": "1〜2文程度の短い概要（検索やOGP用）",
  "source_type": "local",
  "content": "ここにMarkdown形式で本文を記述しますわ。
...
**必須フィールド**:
- `title`, `published_at` (ISO8601推奨), `category` (AI/IT/ガジェット), `score` (1-5), `description`, `source_type`: "local", `content` (Markdown本文)

### 3. 本文（contentフィールド）について

たぬきちゃん風の口調でMarkdownで書いてください。
...
2. **build_articles.py**（ローカル記事）
   - `articles/*.json` を処理（統一スキーマ）
   - JSONの `content` フィールドからHTML生成
